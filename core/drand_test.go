...
func TestDrandDKGReshareTimeout(t *testing.T) {
	...
	for i, drand := range drands[1+offline:] {
		go func(d *Drand, j int) {
			...
			_, err = client.InitReshare(oldPath, newPath, false, timeout, nil)
	...
	go func() {
		...
		require.NoError(t, err)
		_, err = client.InitReshare(oldPath, newPath, true, timeout, nil)
		require.NoError(t, err)
		dkgDone <- true
	}()
...
func TestDrandDKGReshare(t *testing.T) {
	...
	for i, drand := range drands[1:] {
		go func(d *Drand, j int) {
			...
			client, err := net.NewControlClient(d.opts.controlPort)
			require.NoError(t, err)
			_, err = client.InitReshare(oldPath, newPath, false, "", nil)
			require.NoError(t, err)
			wg.Done()
		}(drand, i)
	}
	...
	client, err := net.NewControlClient(root.opts.controlPort)
	require.NoError(t, err)
	_, err = client.InitReshare(oldPath, newPath, true, "", nil)
	require.NoError(t, err)
	//err = root.WaitDKG()
	//require.NoError(t, err)
	wg.Wait()
